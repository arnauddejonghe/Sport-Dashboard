...
(function () {
  ...
  const COLS = [
    ...
    ['rec', 'Récup', (x) => nf(x.rec, 0), 1], ['strain', 'Charge', (x) => nf(x.strain, 1), 1], ['sleepH', 'Sommeil', (x) => fH(x.sleepH), 1],
  ...
  const data = {
    ...
    html() {
      return `<section class="card c6"><div class="card-h"><div><h2>Synchronisation Google Drive</h2><p class="sub">À chaque ouverture, le dashboard cherche tes nouveaux exports dans ton dossier de suivi.</p></div></div><div id="sync-page"></div>
          <p class="note">Fichiers reconnus : Health Export (.csv), MacroFactor (.xlsx), TrainAI (.xlsx), feuille « Retours… » (journal), rapports « AAAA-MM-JJ_coach.md », bilans « Bilan… » (Date;Marqueur;Valeur;Unité;Min;Max).</p></section>
        ...
        <section class="card c12"><div class="card-h"><div><h2>Méthode</h2><p class="sub">Comment chaque indicateur est calculé</p></div></div><dl class="method">
          ...
          <dt>Charge (0–21)</dt><dd>21 × (1 − e^(−charge / 1 100)), où charge = calories actives + 3 × minutes de musculation. Échelle logarithmique : chaque point coûte plus d’effort que le précédent. Cible du jour = 8 + récupération × 0,1 (± 1,5).</dd>
          <dt>Sommeil</dt><dd>Besoin = cible (7 h 30) + 0,06 h par point de charge au-delà de 10 la veille + 25 % de la dette des 3 nuits précédentes (plafond 1 h 30). Performance = sommeil ÷ besoin. Régularité = 100 − écart-type de la durée (min) × 0,83 sur 7 nuits.</dd>
          <dt>Nutrition, activité, note du jour</dt><dd>Nutrition : calories vs cible MacroFactor (0 à ±25 %), protéines vs cible, fibres vs 30 g. Activité : pas vs objectif (70 %) et minutes d’exercice vs 45 (30 %). Note du jour = récup 30 %, sommeil 30 %, nutrition 20 %, activité 20 %.</dd>
          <dt>Note globale</dt><dd>Moyenne pondérée de 6 piliers : récupération 20, sommeil 20 (performance 75 %, régularité 25 %), entraînement 20 (séances vs objectif, séries par muscle dans la fourchette, évolution de l’indice de force), nutrition 15, activité 15, corps 10 (rythme de poids vs cible de la phase). A+ ≥ 90, A ≥ 80, B ≥ 70, C ≥ 60, D ≥ 50.</dd>
          ...
          <dt>Séances et e1RM</dt><dd>Une séance de musculation = un entraînement « Musculation » Apple Santé (sinon TrainAI ou un log MacroFactor) ; doublons fusionnés ; séances de plus de 3 h exclues des durées. e1RM MacroFactor calculé par l’app, TrainAI par la formule d’Epley. Record = e1RM au-dessus de tout l’historique de l’exercice.</dd>
